...
export const NewsletterFooter: React.FC<NewsletterFooterProps> = ({ onNotify, onOpenLegal }) => {
  ...
  return (
    <footer id="club" className="relative bg-[#0E1114] text-white pt-24 pb-12 overflow-hidden border-t border-white/10 select-none">
      
      {/* Background Ambient Glow */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 w-[700px] h-[500px] rounded-full bg-[#E53935]/12 blur-[140px]"
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="max-w-3xl mx-auto text-center mb-20">
          ...
          <h2 className="font-sans font-black text-3xl sm:text-6xl tracking-tight text-white uppercase mb-4 leading-[0.95]">
            UNLOCK 15% OFF YOUR FIRST RITUAL
          </h2>

          <p className="text-sm sm:text-base text-gray-300 max-w-lg mx-auto font-editorial-serif italic mb-8 leading-relaxed">
          ...
          {!isSubscribed ? (
            <form onSubmit={handleSubmit} className="max-w-md mx-auto flex flex-col sm:flex-row items-center gap-3">
              <input
                ...
                className="w-full px-6 py-4 rounded-full bg-white/10 text-white placeholder-gray-400 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E53935] border border-white/20 backdrop-blur-md"
              />
              <button
                type="submit"
            ...
            <motion.div
              ...
              className="bg-white/10 p-6 rounded-3xl max-w-md mx-auto border border-white/25 text-white backdrop-blur-md"
            >
              <div className="flex items-center justify-center gap-2 text-emerald-400 font-sans font-black text-sm mb-1">
                <CheckCircle2 className="w-4 h-4" />
                <span>YOU'RE IN THE FLOW CLUB</span>
              </div>
              <p className="text-xs text-gray-300">
                Use code <span className="font-mono font-bold text-white bg-white/20 px-2 py-0.5 rounded text-sm">LUFF15</span> at checkout for 15% off.
              </p>
            </motion.div>
          )}
        </div>
        ...
        <div className="w-full overflow-hidden select-none my-6 text-center">
          <span className="font-sans font-black text-[22vw] leading-none uppercase tracking-tighter text-white/[0.04] block">
            LUFF
          </span>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-white/10 text-xs text-gray-400">
          <div className="flex flex-wrap items-center justify-center gap-6 font-mono font-semibold uppercase tracking-wider">
            ...
            <button onClick={() => onOpenLegal('shipping')} className="hover:text-white transition-colors">
              Shipping Rates
            </button>
            <button onClick={() => onOpenLegal('returns')} className="hover:text-white transition-colors">
              Returns & Refunds
            </button>
            <button onClick={() => onOpenLegal('admin')} className="text-white/40 hover:text-white transition-colors">
              Admin Portal
            </button>
          </div>

          <div className="text-gray-500 font-mono text-center md:text-right">
